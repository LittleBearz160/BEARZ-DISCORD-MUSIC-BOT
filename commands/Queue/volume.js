...
module.exports = {
	...
	description: "Điều chỉnh âm lượng của nhạc", //the command description for Slash Command Overview
	cooldown: 10,
		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Vui lòng thêm số âm lượng!**`)
				...
				if (volume > 150 || volume < 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Âm lượng phải từ \`0\` đến \`150\`!**`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`🔊 **Đã đổi âm lượng thành \`${volume}\`!**`)

...
module.exports = {
	...
	description: "Hàng chờ hiện tại", //the command description for Slash Command Overview
	cooldown: 10,
			...
			try {
				...
				for (let i = 0; i < theSongs.length; i += 10) {
					...
					const embed = new MessageEmbed()
						...
						.setDescription(`${info}`)
					if (i < 10) {
						embed.setTitle(`📑 **Trang ${theSongs.length > 50 ? 50 : theSongs.length} | Hàng chờ của ${guild.name}**`)
						embed.setDescription(`**(0) Bài nhạc hiện tại:**\n> [\`${theSongs[0].name.replace(/\[/igu, "{").replace(/\]/igu, "}")}\`](${theSongs[0].url})\n\n${info}`)
					}
					embeds.push(embed);
					k += 10; //Raise k to 10
				}
				embeds[embeds.length - 1] = embeds[embeds.length - 1]
					.setFooter(ee.footertext + `\n${theSongs.length} bài hát trong hàng chờ | Thời lượng: ${newQueue.formattedDuration}`, ee.footericon)
				let pages = []
				...
				const Menu = new MessageSelectMenu()
					...
					.addOptions([
						pages.map((page, index) => {
							...
							Obj.value = `${index}`;
							Obj.description = `Hiển thị ${index}/${pages.length - 1} trang!`
							return Obj;
						})
					])

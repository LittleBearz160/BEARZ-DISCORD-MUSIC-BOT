...
module.exports = {
	...
	description: "Chuyển tới một bài nhạc cụ thể trong hàng chờ", //the command description for Slash Command Overview
	cooldown: 10,
		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Vui lòng thêm một vị trí để chuyển đến!**`)
				...
				if (Position > newQueue.songs.length - 1 || Position < 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Vị trí phải từ \`0\` đến \`${newQueue.songs.length - 1}\`!**`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`👌 **Đã chuyển đến bài \`${Position}th\` trong hàng chờ!**`)

...
module.exports = {
	...
	description: "Phát một danh sách kết hợp", //the command description for Slash Command Overview
	cooldown: 2,
	...
	run: async (client, message, args) => {
		try {
			...
			try {
				...
				//Edit the reply
				newMsg.edit({
					content: `${ queue?.songs?.length > 0 ? "👍 đã load xong" : "🎶 Đang phát" }: **'${args[0] ? args[0] : "Default"}'**`,
				});
			} catch (e) {

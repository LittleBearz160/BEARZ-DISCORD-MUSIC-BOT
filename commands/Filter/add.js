...
module.exports = {
	...
	description: "Thêm filter", //the command description for Slash Command Overview
	cooldown: 5,
			...
			try {
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`♨️ **Đã thêm ${toAdded.length} ${toAdded.length == filters.length ? "Filters": `của ${filters.length} Filters! Đã là một phần của Filters!`}**`)

...
module.exports = {
	...
	description: "Xoá 1 Filter từ hàng đợi", //the command description for Slash Command Overview
	cooldown: 5,
			...
			try {
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`♨️ **Đã xoá ${toRemove.length} ${toRemove.length == filters.length ? "Filters": `của ${filters.length} Filters! Đã không còn là một phần của bộ các filter!`}**`)

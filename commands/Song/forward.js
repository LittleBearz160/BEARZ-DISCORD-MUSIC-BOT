...
module.exports = {
	...
	aliases: ["fwd"],
	description: "Tiến thêm ... giây", //the command description for Slash Command Overview
	cooldown: 10,
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [
							...
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Vui lòng thêm 1 thời gian nhất định để tiến thêm!**`)
							.setDescription(`**Usage:**\n> \`${client.settings.get(guild.id, "prefix")}forward <giây>\``)
						],
					})
				}
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`⏩ **Đã tiến thêm \`${seekNumber} giây\`!**`)

...
module.exports = async (client, message) => {
  ...
  if (!cmd || cmd.length == 0) {
    if (mPrefix.includes(client.user.id)) {
      message.reply({
        embeds: [new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle(`:thumbsup: **Prefix của tôi là __\`${prefix}\`__**`)]
      })
    }
    return;
  }
      ...
      if (!botchannels.includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            ...
            .setTitle(`${client.allEmojis.x} **Bạn không được phép sử dụng Lệnh này tại đây!**`)
            .setDescription(`Hãy làm điều đó ở một trong:\n> ${botchannels.map(c=>`<#${c}>`).join(", ")}`)
          ]
        })
      }
    }
      ...
      if (command.maxplusargs && command.maxplusargs > 0 && args.join(` `).split(`++`).filter(Boolean).length > command.maxplusargs) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`${client.allEmojis.x} Sai cú pháp!`)
            .setDescription(command.argstoomany_message && command.argstoomany_message.trim().length > 0 ? command.argsmissing_message : command.usage ? `Usage: ` + command.usage : `Sai cú pháp`)
          ]
        }).then(msg => {
          setTimeout(() => {
            msg.delete().catch((e) => {
